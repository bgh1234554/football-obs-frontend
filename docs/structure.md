...
| `Space` | 타이머 시작/정지 |
| `R` | 타이머 리셋 (00:00) |
| `[` / `]` | 전/후반 이전/다음 (`1H → 2H → ET1 → ET2 → PK`) |
| `H` | 탭바 숨기기/표시 토글 |
| `\` | 전술판 전체화면 토글 (전술판 탭 활성화 시에만) |
| `1` ~ `6` | 탭 직접 전환 (메인 큰/작은, 테마, 일정, 전술판, 어바웃) |
| `7` | 경기 ID 입력 오버레이 열기 |
| `8` | Buy me a coffee 새 탭 열기 |
| `Ctrl+Z` | 전술판 Undo |
| `Ctrl+Y` | 전술판 Redo |
| `Q` / `A` | 수동 모드: 홈 점수 +1 / -1. PK 하프: 홈 골(G) / 실축(M) |
| `W` / `S` | 수동 모드: 어웨이 점수 +1 / -1. PK 하프: 어웨이 골(G) / 실축(M) |
| `F` | 수동 모드: 홈/어웨이 점수 초기화 |
| `T` | 수동 모드: 추가시간 표시/숨김 토글 |
| `Z` | PK 하프: 마지막 킥 결과 Undo |
| `X` | PK 하프: PK 결과 전체 초기화 |
